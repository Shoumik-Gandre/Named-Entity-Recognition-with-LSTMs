...
from ..dataset import Conll03Dataset, task2_collate, word_preprocessor, case_feature_extractor
...
def train_model(train_path: PathLike, dev_path: PathLike, temp_pred_path: PathLike, checkpoint_path: PathLike):
    ...
    train_dataloader = DataLoader(train_dataset, batch_size=BATCH_SIZE, 
                                  shuffle=SHUFFLE, collate_fn=partial(task2_collate, use_targets=True))
    ...
    dev_dataloader = DataLoader(dev_dataset, batch_size=BATCH_SIZE, 
                                collate_fn=partial(task2_collate, use_targets=True))
    ...
    for epoch in range(start_epoch, NUM_EPOCHS):
        ...
        print("=> [Training]")
        train_loss = train_step(model, criterion, optimizer, scheduler, train_dataloader, device)
